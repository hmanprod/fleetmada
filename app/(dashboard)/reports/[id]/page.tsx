...
import { ArrowLeft, Save, Share2, Download, Printer, Filter, ChevronDown, ChevronRight } from 'lucide-react';
...
const mockReports: Report[] = [
  { id: 'operating-costs', title: 'Operating Cost Summary' },
  { id: 'expense-summary', title: 'Expense Summary Report' },
];
...
export default function ReportDetailPage({ params }: { params: { id: string } }) {
  ...
  if (!report) {
    return (
      <div className="p-8">
        <button onClick={() => router.push('/dashboard/reports')} className="text-gray-500 hover:text-gray-700 flex items-center gap-1 mb-4">
          <ArrowLeft size={16}/> Reports
        </button>
        <h1 className="text-2xl font-bold">Report not found</h1>
        <p className="mt-4 text-gray-500">The requested report could not be found.</p>
      </div>
    );
  }
  ...
  const handleExport = () => {
    console.log('Export CSV:', report.id);
  };
  ...
  // A simplified placeholder for other reports
  if (!isOperatingCost && report.id !== 'expense-summary') {
    return (
      <div className="p-8">
        <button onClick={handleBack} className="text-gray-500 hover:text-gray-700 flex items-center gap-1 mb-4">
          <ArrowLeft size={16}/> Reports
        </button>
       ...
       <div className="bg-white border-b border-gray-200 px-8 py-4 sticky top-0 z-10 flex justify-between items-center">
          <div>
             <div className="text-sm text-gray-500 mb-1 flex items-center gap-2">
               <button onClick={handleBack} className="hover:underline">Reports</button> / {report.title}
             </div>
          </div>
          <div className="flex gap-2">
             <button onClick={handleSave} className="px-3 py-1.5 border border-gray-300 bg-white rounded text-sm font-medium text-gray-700 hover:bg-gray-50 flex items-center gap-2">
                <Save size={16} /> Save
             </button>
             <button onClick={handleShare} className="px-3 py-1.5 border border-gray-300 bg-white rounded text-sm font-medium text-gray-700 hover:bg-gray-50 flex items-center gap-2">
                <Share2 size={16} /> Share
             </button>
             <button onClick={handleExport} className="px-3 py-1.5 border border-gray-300 bg-white rounded text-sm font-medium text-gray-700 hover:bg-gray-50 flex items-center gap-2">
                <Download size={16} /> Export CSV <ChevronDown size={14} />
             </button>
             <button onClick={handlePrint} className="px-3 py-1.5 border border-gray-300 bg-white rounded text-sm font-medium text-gray-700 hover:bg-gray-50 flex items-center gap-2">
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                        ...
                        {mockData.slice(0, 3).map((row, idx) => (
                            <tr key={idx} className="hover:bg-gray-50">
                                <td 
                                  className="px-6 py-3 text-[#008751] hover:underline cursor-pointer font-medium"
                                  onClick={() => handleVehicleClick(row.vehicle)}
                                >
                                  {row.vehicle}
                                </td>
