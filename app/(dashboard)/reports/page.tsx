...
export default function ReportsPage() {
  ...
  const handleReportTypeFilter = (category: string) => {
    console.log('Filter by category:', category);
  };

  const reportCategories = [
    { id: 'vehicles', name: 'Vehicles', icon: Car, count: 13 },
    { id: 'assignments', name: 'Vehicle Assignments', icon: ClipboardCheck, count: 2 },
    { id: 'inspections', name: 'Inspections', icon: ClipboardCheck, count: 4 },
    { id: 'issues', name: 'Issues', icon: AlertTriangle, count: 2 },
    { id: 'service', name: 'Service', icon: Wrench, count: 8 },
    { id: 'work-orders', name: 'Work Orders', icon: Wrench, count: 4 },
    { id: 'contacts', name: 'Contacts', icon: Users, count: 2 },
    { id: 'parts', name: 'Parts', icon: Box, count: 1 },
    { id: 'fuel', name: 'Fuel', icon: Fuel, count: 3 },
  ];
               ...
               <div className="space-y-1">
                   {reportCategories.map(category => {
                     ...
                     return (
                       <button 
                         key={category.id}
                         onClick={() => handleReportTypeFilter(category.id)}
                         className="w-full flex items-center justify-between py-1.5 text-sm text-gray-600 hover:text-gray-900"
                       >
                           <span className="flex items-center gap-2"><Icon size={16}/> {category.name}</span>
           ...
           <div className={viewMode === 'grid' ? 'grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4' : 'space-y-4'}>
               {filteredReports.map(report => (
                   <div 
                    ...
                    className={`bg-white rounded-lg border border-gray-200 shadow-sm hover:border-[#008751] hover:shadow-md transition-all cursor-pointer flex flex-col ${
