...
import { ChevronDown, Loader2, ChevronLeft, Upload, Settings2, X, Camera, FileText, Trash2 } from 'lucide-react';
...
export default function WorkOrderEditPage({ params }: { params: { id: string } }) {
    ...
    const { updateWorkOrder, loading: saving } = useServiceWorkOrders();
        ...
        const fetchWorkOrder = async () => {
            try {
                ...
                if (response.success && response.data) {
                    ...
                    const dateValue = wo.date ? new Date(wo.date).toISOString().split('T')[0] : '';
                    const timeValue = wo.date ? new Date(wo.date).toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' }) : '';
                    ...
                    setFormData(prev => ({
                        ...
                        date: dateValue,
                        time: timeValue,
                        issuedBy: wo.issuedBy || wo.user?.name || 'Hery RABOTOVAO',
                        scheduledStartDate: wo.scheduledStartDate || dateValue,
                        scheduledStartTime: wo.scheduledStartTime || timeValue,
                        invoiceNumber: wo.invoiceNumber || '',
                        poNumber: wo.poNumber || '',
                        meter: wo.meter?.toString() || '',
                        discountValue: wo.discountValue || 0,
                        discountType: wo.discountType || '%',
                        taxValue: wo.taxValue || 0,
                        taxType: wo.taxType || '%',
                        assignedToContactId: wo.assignedTo || '',
                        vendorId: (typeof wo.vendor === 'object' ? wo.vendor?.id : wo.vendor) || '',
                        notes: wo.notes || '',
                        tasks: wo.tasks?.map(t => ({
                            id: t.id,
                            entityId: t.serviceTask.id,
                        ...
                        parts: wo.parts?.map(p => ({
                            id: p.id,
                            entityId: p.part.id,
                            name: p.part.description,
                            cost: p.unitCost || 0,
                            notes: p.notes || ''
                        })) || [],
                    }));
                } else {
                    toast.error('Erreur', 'Impossible de charger l\'ordre de travail');
                }

                // Fetch photos
                const photosResponse = await serviceAPI.getServiceEntryPhotos(params.id);
                if (photosResponse) {
                    const existingPhotos = photosResponse.map((p: any) => ({
                        id: p.id,
                        name: p.fileName,
                        type: 'photo' as const,
                        url: p.filePath
                    }));
                    setDocList(prev => [...prev, ...existingPhotos]);
                }
            } catch (error) {
                console.error('Error fetching work order:', error);
                toast.error('Erreur', 'Erreur lors du chargement des données');
            } finally {
                setIsLoading(false);
            }
        };
    ...
    const handleSave = async () => {
        ...
        try {
            const result = await updateWorkOrder(params.id, {
            ...
            if (result) {
                toast.success('Succès', 'Ordre de travail mis à jour avec succès');
                setTimeout(() => router.push(`/service/work-orders/${params.id}`), 1500);
            }
        } catch (error) {
            toast.error('Erreur', 'Impossible de mettre à jour l\'ordre de travail');
        }
    };
    ...
    const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>, type: 'photo' | 'document') => {
        ...
        uploadData.append('attachedTo', params.id);
            ...
            <div className="bg-white border-b border-gray-200 px-8 py-4 sticky top-0 z-30 flex justify-between items-center shadow-sm">
                <div className="flex items-center gap-6">
                    <button onClick={handleBack} className="text-gray-500 hover:text-gray-700 flex items-center gap-1 font-medium text-sm">
                        <ChevronLeft size={18} /> Retour à l'ordre
                    </button>
                    <div className="flex items-center gap-2">
                        <h1 className="text-xl font-bold text-gray-900">Modifier l'ordre de travail</h1>
                        <span className="text-sm text-gray-400 font-medium">#{params.id.slice(-6).toUpperCase()}</span>
                    </div>
                </div>
                            ...
                            <div className="flex gap-6">
                                {(['SCHEDULED', 'IN_PROGRESS', 'COMPLETED', 'CANCELLED'] as const).map((status) => (
                                    <label key={status} className="flex items-center gap-2 cursor-pointer group">
                                        ...
                                        />
                                        <span className="text-sm font-medium text-gray-900 group-hover:text-[#008751] transition-colors">
                                            {status === 'SCHEDULED' ? 'Ouvert' : status === 'IN_PROGRESS' ? 'En cours' : status === 'COMPLETED' ? 'Terminé' : 'Annulé'}
                                        </span>
                                    </label>
                                ))}
                            </div>
