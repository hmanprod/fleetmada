...
export default function ServiceHistoryPage() {
  ...
  const handleAddServiceEntry = () => {
    router.push('/dashboard/service/create');
  };

  const handleEntryClick = (entryId: number) => {
    // TODO: Navigate to service entry detail page
  ...
  const handleSelectAll = () => {
    if (selectedEntries.length === mockHistory.length) {
      setSelectedEntries([]);
    } else {
      setSelectedEntries(mockHistory.map(entry => entry.id));
    }
  };
          ...
          <thead className="bg-gray-50">
            <tr>
              <th className="w-8 px-6 py-3">
                 <input 
                   type="checkbox" 
                   checked={selectedEntries.length === mockHistory.length && mockHistory.length > 0}
