...
test.describe('Module Parts - Tests E2E', () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    await page.fill('[data-testid="email"]', 'admin@fleetmada.com')
    await page.fill('[data-testid="password"]', 'admin123')
    await page.click('[data-testid="login-button"]')
    
    // Attendre la connexion et récupérer le token
  ...
  test('Devrait afficher la page des pièces avec la liste vide', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="tab-low-stock"]')).toContainText('Low Stock')
    
    // Vérifier le message de liste vide
    await expect(page.locator('text=Aucune pièce trouvée')).toBeVisible()
    await expect(page.locator('text=Ajouter une pièce')).toBeVisible()
    
    // Vérifier les filtres
    await expect(page.locator('input[placeholder="Search"]')).toBeVisible()
    await expect(page.locator('button:has-text("Part Category")')).toBeVisible()
    await expect(page.locator('button:has-text("Part Manufacturer")')).toBeVisible()
  })

  test('Devrait créer une nouvelle pièce', async ({ page }) => {
  ...
  test('Devrait afficher les détails d\'une pièce', async ({ page }) => {
  ...
  test('Devrait éditer une pièce existante', async ({ page }) => {
  ...
  test('Devrait gérer le stock avec ajustements', async ({ page }) => {
  ...
  test('Devrait filtrer les pièces par catégorie', async ({ page }) => {
  ...
  test('Devrait rechercher des pièces', async ({ page }) => {
    // Créer des pièces
    const pieces = [
  ...
  test('Devrait afficher les onglets All et Low Stock', async ({ page }) => {
  ...
  test('Devrait afficher les statistiques en bas de page', async ({ page }) => {
  ...
  test('Devrait naviguer correctement avec les boutons retour', async ({ page }) => {
